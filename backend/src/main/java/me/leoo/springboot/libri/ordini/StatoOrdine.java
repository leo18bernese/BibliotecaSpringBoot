...
public enum StatoOrdine {
    ...
    IN_ATTESA("In attesa", "L'ordine è stato ricevuto e in attesa di elaborazione.", "A breve gli articoli saranno raccolti e preparati per la spedizione.", false),
    IN_PREPARAZIONE("In preparazione", "L'ordine è in fase di preparazione.", "Gli articoli sono in fase di raccolta e imballaggio.", true),
    SPEDITO("Spedito", "L'ordine è stato spedito e in transito verso la destinazione.", "Puoi tracciare il tuo ordine con il numero di tracciamento fornito.", true),
    ...
    public static StatoOrdine[] getUpdatableStates() {
        return Arrays.stream(values()).filter(stato -> stato.updatableByAdmin).toArray(StatoOrdine[]::new);
    }
}
